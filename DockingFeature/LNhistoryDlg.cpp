...
//BM_SETCHECK BM_SETCHECK:MF_UNCHECKED
    ...
    ::SendMessage( _hAlways, BM_SETCHECK, ( LPARAM )( AlwaysRecord ? 1 : 0 ),
                   0 );
    ::SendMessage( _hSaveRecord, BM_SETCHECK, ( LPARAM )( SaveRecord ? 1 : 0 ),
                   0 );
...
void EnableTBButton( menuList flagIndex, bool state )
{
    if ( menuState[flagIndex] != state )
    {
        ...
        ::EnableMenuItem( ::GetMenu( nppData._nppHandle ),
                          funcItem[flagIndex]._cmdID,
                          MF_BYCOMMAND | ( state ? MF_ENABLED : MF_GRAYED ) );

        if ( IconID[flagIndex] != -1 )
        {
            ::SendMessage( hToolbar, TB_ENABLEBUTTON, ( WPARAM )IconID[flagIndex],
                           MAKELONG( ( state ? TRUE : FALSE ), 0 ) );
...
void SetPosByIndex()
{
    ...
    if ( pos != -1 && res == 1 )
    {
        ...
        // ??????,????
        // ?????????,?????
        long beginLines = ::SendMessage( curScintilla, SCI_GETFIRSTVISIBLELINE, 0,
                                         0 );
        // ?????????
        //long currLines = ::SendMessage(curScintilla, SCI_GETCURLINE , 0 , 0);
        //long position = ::SendMessage(curScintilla, SCI_GETCURRENTPOS, 0, 0);
        long targetLine = ::SendMessage( curScintilla, SCI_LINEFROMPOSITION, pos,
                                         0 );
        // ??????
...
INT_PTR CALLBACK LocationNavigateDlg::run_dlgProc( UINT message,
    ...
    switch ( message )
    {
        case WM_COMMAND :
        {
            switch ( wParam )
            {
                ...
                {
                    BOOL isSuccessful;
                    int val = ::GetDlgItemInt( _hSelf, ID_LNHISTORY_EDIT, &isSuccessful,
                                               FALSE );
        ...
        case SELF_REFRESH://WM_TIMER
        {
            ...
            for ( size_t i = 0; i < LocationList.size(); i++ )
            {
                ...
                wsprintf( strHint, TEXT( "%c,%d,%s" ), LocationList[i].changed ? '!' : '=',
                          LocationList[i].position + 1, LocationList[i].FilePath );
        ...
        case WM_INITDIALOG:
        {
            ...
            ::SendMessage( _hBookmark, CB_ADDSTRING, 0,
                           ( LPARAM )TEXT( "Mark with highlight" ) );
            ::SendMessage( _hBookmark, CB_ADDSTRING, 0,
                           ( LPARAM )TEXT( "Mark with underline" ) );
            ::SendMessage( _hBookmark, CB_ADDSTRING, 0,
                           ( LPARAM )TEXT( "Mark with rect" ) );
            ::SendMessage( _hBookmark, CB_ADDSTRING, 0,
                           ( LPARAM )TEXT( "Mark with roundrect" ) );
            ::SendMessage( _hBookmark, CB_ADDSTRING, 0,
                           ( LPARAM )TEXT( "Mark with circle" ) );
            ::SendMessage( _hBookmark, CB_ADDSTRING, 0,
                           ( LPARAM )TEXT( "Mark with triangle" ) );
            ::SendMessage( _hBookmark, CB_ADDSTRING, 0,
                           ( LPARAM )TEXT( "Mark with arrow" ) );
            ::SendMessage( _hBookmark, CB_ADDSTRING, 0,
                           ( LPARAM )TEXT( "Mark with bookmark" ) );
        ...
        case WM_MOVE:
        {
            ...
            ::MoveWindow( _hListBox, rc.left, rc.top, rc.right, rc.bottom - 100, TRUE );
