...
LRESULT SelfCtrl::runProc( HWND hwnd, UINT Message, WPARAM wParam,
                           LPARAM lParam )
{
    switch ( Message )
    {
        ...
        {
            if ( _hCursor == 0 )
                _hCursor = ::CreateCursor( ::GetModuleHandle( 0 ), 5, 2, 32, 32, XORMask,
                                           ANDMask );
        ...
        case WM_LBUTTONUP:
            if ( _clicking )
            {
                ...
                // Open a color
                CHOOSECOLOR
                cc;                                   //   common   dialog   box   structure
                static   COLORREF   acrCustClr[16];   //   array   of   custom   colors
                static   DWORD
                rgbCurrent;                 //   initial   color   selection
                ...
                if   ( ChooseColor( &cc ) == TRUE )
                {
                    ...
                    {
                        MarkColor = _bkColor;
                        ::SendMessage( curScintilla, SCI_MARKERSETBACK, _MARK_INDEX,
                                       _bkColor ); // 默认颜色
                    }
                    else if ( SelfID == _SAVE_INDEX )
                    {
                        SaveColor = _bkColor;
                        ::SendMessage( curScintilla, SCI_MARKERSETBACK, _SAVE_INDEX,
                                       _bkColor ); // 保存颜色
                    }
